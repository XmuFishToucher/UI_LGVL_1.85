...
void LCD_Init() {
  I2C_Init();  // Initialize I2C for EXIO (TCA9554)
  ST77916_Init();
  Backlight_Init();
  Touch_Init();
}
...
int QSPI_Init(void){
  ...
  lcd_io_handle = NULL;
  if(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)ESP_PANEL_HOST_SPI_ID_DEFAULT, &io_config, &lcd_io_handle) != ESP_OK){
  ...
  ret = esp_lcd_panel_io_rx_param(lcd_io_handle, lcd_cmd, register_data, param_size); 
  if (ret == ESP_OK) {
  ...
  esp_lcd_new_panel_st77916(lcd_io_handle, &panel_config, &panel_handle);
